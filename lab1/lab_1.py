...
X_ames_train = sm.add_constant(X_ames_train)

model = sm.OLS(y_ames_train, X_ames_train).fit()
model_l1_params = sm.OLS(y_ames_train, X_ames_train).fit_regularized(L1_wt=1.0, alpha=reg_lasso_alpha).params
model_l1 = sm.OLS(y_ames_train, X_ames_train).fit()
model_l1.params[:] = model_l1_params
model_l2_params = sm.OLS(y_ames_train, X_ames_train).fit_regularized(L1_wt=0.0, alpha=reg_ridge_alpha).params
...
_, _, _, p_l2 = sms.het_breuschpagan(model_l2.resid, model_l2.model.exog)
...
# %% [markdown]
# Niestety nie wiem czemu p-wartosc w pierwszym przypadku jest tak duza w porownaniu do regularyzacji
...
# %%
bins_normal = np.arange(-1, 1, 0.05)
bins_sus = np.arange(-1, 3, 0.1)

plt.hist(model.resid, bins=bins_normal, edgecolor='black')
plt.title("No regularization")
plt.show()
plt.hist(model_l1.resid, bins=bins_normal, edgecolor='black')
plt.title("l1 regularization")
plt.show()
plt.hist(model_l2.resid, bins=bins_sus, edgecolor='black')
plt.title("l2_regularization")
plt.show()

# %% [markdown]
# Dlaczego rozklad rezyduów l2 nie przypomina rozkładu normalnego a mimo to test zwraca małą wartośc p?
